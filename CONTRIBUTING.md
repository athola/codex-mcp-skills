...
- Public API guard: run `cargo +nightly public-api diff --manifest-path crates/server/Cargo.toml --deny removed --deny changed origin/master..HEAD` before opening a PR. CI enforces the same check.
