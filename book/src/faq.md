...
Most alternatives fall into a few select categories: static skill bundles for manual copy, CI pipelines that render SKILL-like docs into prompts, shared rule repositories, local-only sync CLIs, or tutorials. codex-mcp-skills adds an MCP server built in Rust for performance and memory safety, a Codex hook, cross-agent sync capabilities, and turnkey installers so skills are served as runtime resources.
