...
- Public API guard: run `cargo +nightly public-api -p skrills-server --deny removed --deny changed --baseline-git-ref origin/master` before opening a PR. CI enforces the same check.
