...
- Public API guard: run `cargo public-api diff -p skrills-server --deny removed --deny changed origin/master..HEAD` before opening a PR. CI enforces the same check.
